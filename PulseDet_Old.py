...
def interpolationAndFiltering(inputMatrix, inFr, outFr, lowerCutoff=0.75, higherCutoff=4, filterOrder=5, savgolWindow = 61, polynomialOrder=7):
    ...
    for i in range(columns):
        ...
        ###################
        # nyquist = len(inputCol)
        # x_disp = 15*np.arange(nyquist)/nyquist
        # x_disp = x_disp[1:]
        # y_disp = np.fft.fft(inputCol)
...
if __name__ == '__main__':
    ...
    cap = cv2.VideoCapture(os.path.join(ROOT,video_filenames[2]))
